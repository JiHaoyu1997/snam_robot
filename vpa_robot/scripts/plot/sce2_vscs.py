...
# 读取 Excel 文件
df = pd.read_excel(file_path, sheet_name="sce2vscs")

# 只保留两列，假设它们是 "agent1" 和 "agent2"
...
plt.axhline(y=avg_agent2, color="green", linestyle="--", linewidth=1.5, label=f"Avg Agent3 Travel Time: {avg_agent3:.2f}s")

# 标注平均值
plt.text(0.7, avg_agent1+0.1, f"{avg_agent1:.2f}s", verticalalignment='bottom', fontsize=10, color="blue")
plt.text(len(df)/2, avg_agent2, f"{avg_agent2:.2f}s", verticalalignment='bottom', fontsize=10, color="orange")
plt.text(len(df)-2.5, avg_agent2-0.1, f"{avg_agent3:.2f}s", verticalalignment='bottom', fontsize=10, color="green")
...
plt.text(min_agent1_idx, min_agent1, f"Agent 1 Min {min_agent1:.2f}s", verticalalignment='top', fontsize=10, color="blue")

plt.scatter(min_agent2_idx, min_agent2, color="orange", marker='s', s=80, edgecolors="black", zorder=3)
plt.text(min_agent2_idx, min_agent2, f"Agent 2 Min {min_agent2:.2f}s", verticalalignment='top', fontsize=10, color="orange")

plt.scatter(min_agent3_idx, min_agent3, color="green", marker='*', s=80, edgecolors="black", zorder=3)
plt.text(min_agent3_idx, min_agent3, f"Agent 3 Min {min_agent3:.2f}s", verticalalignment='top', fontsize=10, color="green")

# 添加标题和标签
plt.title("Travel Time under VSCS")
plt.xlabel("Experiment Number")  # 修改横坐标标签
plt.ylabel("Travel Time (s)")

# 显示图例
plt.legend()
plt.grid(True)
# plt.ylim(0, 10)  # 设置 y 轴范围从 0 到 10
